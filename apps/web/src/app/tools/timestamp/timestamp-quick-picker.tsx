...
const TIMESTAMP_QUICK_PICKER_OPTIONS: QuickPickerOption[] = [
  ...
  {
    label: "×1000",
    getValue: ({ timestamp, granularityValue }) => `${timestamp}${granularityValue}000`,
  },
  {
    label: "÷1000",
    getValue: ({ timestamp, granularityValue }) => `${timestamp}${granularityValue}`.slice(0, -3),
  },
  {
    label: "Round",
    getValue: ({ timestamp, granularity }) =>
      `${timestamp}${"0".repeat(TIMESTAMP_GRANULARITY_VALUE_LENGTH[granularity])}`,
...
export function TimestampQuickPicker({
  ...
  return (
    <div className="flex gap-2">
      {TIMESTAMP_QUICK_PICKER_OPTIONS.filter(({ disabled }) => !disabled?.(granularity)).map(
        ({ label, getValue }) => (
          <Button
            key={label}
